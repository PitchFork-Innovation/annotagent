...
- Data: MongoDB Atlas (Mongoose), AWS S3 object storage, NextAuth.js v5 (Credentials + JWT), Resend email, TTL-indexed chat persistence in MongoDB.
...
- Keep contracts synchronized across boundaries. If you change schema or payload shape, inspect the relevant Mongoose model in `lib/models/`, `lib/types.ts`, `lib/ingestion-schema.ts`, `lib/server-data.ts`, route handlers, and Python models/prompts together.
...
- Preserve AI/data invariants: short exact `text_ref` values, validated annotation output, one paper per `arxivId`, and graceful fallback when chat TTL persistence is unavailable.
